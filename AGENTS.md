...
- Follow git-flow for branch management: use `develop` as the integration branch and keep `master` production-ready.
- Create `feature/*` branches from `develop` and merge them back into `develop` through pull requests.
- Create `release/*` branches from `develop`, then merge completed releases into both `master` and `develop`.
- Create `hotfix/*` branches from `master`, then merge completed hotfixes into both `master` and `develop`.
