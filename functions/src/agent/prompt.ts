...
- ユーザが目的駅を明示した場合、その駅の検索が 1 回 0 件だっただけで
  「到達できない」「経路を確認できない」と回答してはならない。
  残りのツール呼び出し回数がある限り、同じ目的駅を漢字・かな・読み・特徴的な部分で
  少なくとももう 1 回検索し、直通だけでなく複数の列車種別を跨ぐ候補を確認する
...
export const buildContextMessage = (
  ...
  if (currentStation) {
    ...
    lines.push(
      ...
      '駅検索は現在駅から直通、または列車種別を連結して到達できる駅だけを返す。目的駅が明示されている場合は 1 回の 0 件で到達不能と判断せず、同じ目的駅を別表記・読み・特徴的な部分でもう一度検索する'
    );
