...
- search_stations_by_name が連結経路付きの駅候補を返した場合、その駅へ到達できることを肯定的に案内し、その行き先駅を suggestions に含める。経路上の途中駅を別の行き先候補として追加してはならない。「候補を出せない」「到達できない」など、ツール結果と矛盾する返答をしてはならない
